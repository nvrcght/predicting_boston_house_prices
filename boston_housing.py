...
def explore_city_data(city_data):
    ...
    print "There are %d houses in a data set" %housing_features.shape[0]
    print "There are %d features in a data set" %housing_features.shape[1]
    print "Minimum price of a house: $%dK" %(housing_prices.min())
    print "Maximum price of a house: $%dK" %(housing_prices.max())
    print "Mean price of a house: $%dK" %(housing_prices.mean())
    print "Median price of a house: $%dK" %(np.median(housing_prices))
    print "Standart deviation $%0.2fK" %(np.std(housing_prices))

def performance_metric(label, prediction):
    """Calculate and return the appropriate error performance metric."""

    mean_sq_error = mean_squared_error(label, prediction)
